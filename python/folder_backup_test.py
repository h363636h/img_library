...
sys.path.append("/home/crystal/MG/asset_library/python/change_path")
...
class Folder_backup:
    def __init__(self):
        ...
        self.createDir = "//home/crystal/Desktop/project_test/"
    ...
    def prj(self,baseDir):
        try :
            ...
            if not os.path.exists(self.createDir + self.project):
                os.umask(0)
                os.mkdir(self.createDir + self.project,0777)
            else:
                print "folder exists"
                pass
    ...
    def model(self,project,dir,model):
        ...
        if os.path.isdir(model_dir):   #model 폴더 존재할 경우 처리
            ...
            for model_cate in model_cate_arr:   #model 내 category list 처리
                create_model_cate = self.createDir + project + "/" + model_cate
    ...
    def texture(self,project,dir,texture):
        ...
        if os.path.isdir(texture_dir):  #texture 폴더 존재할 경우 처리
            ...
            for texture_cate in texture_cate_arr:   #texture 내 category list 처리
                create_texture_cate = self.createDir + project + "/" + texture_cate
    ...
    def rigging(self,project,dir,rigging):
        ...
        if os.path.isdir(rigging_dir):  #rigging 폴더 존재할 경우 처리
            ...
            for rigging_cate in rigging_cate_arr:   #rigging 내 category list 처리
                create_rigging_cate = self.createDir + project + "/" + rigging_cate
    ...
    def lookdev(self,project,dir,lookdev):
        ...
        if os.path.isdir(lookdev_dir):  #lookdev 폴더 존재할 경우 처리
            ...
            for lookdev_cate in lookdev_cate_arr:   #lookdev 내 category list 처리
                create_lookdev_cate = self.createDir + project +"/" + lookdev_cate
...
class DB_update:
    ...
    def insert(self,prj):
        assetALL = sg.find('Asset', [['project.Project.name', 'is', prj]], ['image', 'code'])
                ...
                for asset2 in asset_arr:
                    if asset_name == asset2:
                        ...
                        for lod in lod_arr:
                            ...
                            query = "insert into asset(asset_name,path,full_path,thum,lod,texture,model,rigging,lookdev,project) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                            asset_name, dir_cate + "/", dir_asset + "/", thum, lod, texture, model, rigging, lookdev,
                            project)
