...
mov_path = "/home/crystal/MG/asset/thumnail/mov/"
...
class Folder_backup:
    ...
    def prj(self,baseDir):
        try :
            ...
            if not os.path.exists(createDir + project):
                os.umask(0)
                os.mkdir(createDir + project,0777)
            else:
                print "folder exists"
                pass
            ...
            if os.path.isdir(dir):
                ...
                for mtrl in mtrl_arr:
                    if mtrl == "model":
                        pass
                        self.model(project,dir,mtrl)
                    elif mtrl == "texture":
                        pass
                        self.texture(project,dir,mtrl)
                    elif mtrl == "rigging":
                        pass
                        self.rigging(project,dir,mtrl)
                    elif mtrl == "lookdev":
                        pass
                        self.lookdev(project,dir,mtrl)
            else:
...
class DB:
    ...
    def insert(self,project):
        ...
        for asset in assetALL:
            ...
            thum_img = "%s_" % (datetime.today().strftime('%Y%m%d')) + str(uuid.uuid4())[:8] + ".jpg"
            thum_path = "/home/crystal/MG/asset/thumnail/small_thum/%s" % thum_img

            asset_name = asset['code']  #asset name
            asset_thum = str(asset['image'])    #asset thumbnail path

            origin_thum = asset_thum


            cate_arr = os.listdir(createDir + project)  #해당 어셋의 t,m,r,l 존재 여부 확인하기 위함
            for cate in cate_arr:
                dir_cate = createDir + project + "/" + cate
                asset_arr = os.listdir(dir_cate)
                for asset2 in asset_arr:
                    if asset_name == asset2:
                        dir_asset = dir_cate + "/" + asset_name
                        ...
                        for lod in lod_arr:
                            ...
                            select_query = "select * from asset where asset_name='%s' and lod='%s' and project='%s'" % (
                                asset_name, lod, project)
                            cur.execute(select_query)
                            select_rows = cur.fetchall()

                            # print select_rows

                            if select_rows == ():   #db에 asset 정보 insert
                                ...
                                if lookdevALL == None:  #lookdev 없을 경우, asset thumbnail db insert
                                    query = "insert into asset(asset_name,path,full_path,thum,mov,lod,texture,model,rigging,lookdev,project,mod_path,update_date) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','X','%s')" % (
                                    asset2, dir_cate + "/", dir_asset + "/", thum_img, "None", lod,
                                    texture, model, rigging, lookdev, project,today)
                                    ...
                                    mov_filename = lookdev_mov.split("/")[-1]
                                    if mov_filename == "None":
                                        convert_codec = "None"

                                    else:

                                        convert_codec = "/home/crystal/MG/asset/thumnail/mov/2_%s" % mov_filename

                                        try:
                                            shutil.copy(lookdev_mov, mov_path)
                                        except:
                                            pass

                                        ext = os.path.splitext(lookdev_mov)[-1]
                                        ...
                                        if ext.lower() == ".png":
                                            ...
                                            try:
                                                ...
                                                ff = FFmpeg(
                                                    inputs={'%s'%(mov_path+mov_filename): None},
                                                    outputs={'%s'%(convert_codec): '-c:v h264 -c:a ac3'}
                                                )
                                                ff.run()
                                                os.remove(mov_path+mov_filename)
                                            except Exception, e:
                                                print e
                                                pass

                                    lookdev_thum = str(lookdevALL['image'])
                                    origin_thum = lookdev_thum

                                    query2 = "insert into asset(asset_name,path,full_path,thum,mov,lod,texture,model,rigging,lookdev,project,mod_path,update_date) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','X','%s')" % (
                                    asset2, dir_cate + "/", dir_asset + "/", thum_img, convert_codec, lod,
                                    texture, model, rigging, lookdev, project, today)
                            ...
                            thumbnail_file.retrieve(origin_thum,thum_path)
